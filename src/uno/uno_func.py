...
Rule = "★ハウスルール(基本的なものは除く)\n" \
       ...
       "8-3. ワイルドディスカードオールは、消したい色を出した枚数分指定できる(重複可能)\n" \
       "8-4. ディスカードオール同士は組み合わせて出すことが可能\n" \
       ...
       "・ワイルドカード → ワイルド [W], ワイルドディスカードオール [WDA], ドロー4 [+4, D4], ドボン1 [+1, D1], ドボン2 [+2, D2]\n\n" \
...
for n1 in Number:
    for n2 in Color:
        Card.append(f"{n2}{n1}")
Card = (Card * 2)[4:-4] + ["ワイルド", "ドロー4"] * 4 + ["ドボン1", "ワイルドディスカードオール"] * 2 + ["ドボン2"]
Card_Normal = Card
...
def template_check(stc):
    ...
    # カードの枚数設定に応じて新カードを生成
    mark = ["0", "7", "9", "スキップ", "リバース", "ドロー2", "ディスカードオール",
            "ワイルド", "ワイルドディスカードオール", "ドロー4", "ドボン1", "ドボン2"]
    marks, new_card = [], []
    for i in range(7):
...
def translate_input(word):
    if word in ["w", "wild", "ヒカキン", "hikakin"]:
        ...
        return "ドボン2"
    elif word in ["wda", "wilddiscardall", "w修造", "ワイルド修造"]:
        return "ワイルドディスカードオール"
    else:
...
# ドボン1 550～554, ドボン2 560～564, ワイルドディスカードオール 570～574
def card_to_id(card):
    if "ワイルドディスカードオール" in card:
...
def check_card(before, after, hand, penalty):
    ...
    elif "ディスカードオール" in after[0]:
        # ディスカードオール以外が含まれているか判定
        if not all(["ディスカードオール" in i for i in after]):
            return "ディスカードオールと一緒に出せるのは、ディスカードオールのみです"
    else:
    ...
    # 色は異なるが記号が一致
        ...
        pass
    # ワイルドディスカードオールの後にディスカードオールを出す
    elif 570 <= before <= 574 and first % 100 == 13:
        pass
    else:
