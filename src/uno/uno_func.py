...
Rule = "★ハウスルール(基本的なものは除く)\n" \
       ...
       "5-3. 他プレイヤーのUNO宣言忘れを指摘する際は メンション !UNO と入力する (例 : @そばゆ !UNO)\n" \
       "5-4. UNOとなる人のターンが終了してから10秒間は指摘出来ない\n" \
       "5-5. UNOと宣言せずに上がる or 指摘されると2枚ペナルティー\n\n\n" \
       "★その他ゲーム中のコマンド\n" \
       "・ゲームに途中参加する → !Join\n" \
...
def translate_input(word):
    if word in ["w", "wild", "ドリラー"]:
        ...
        return "ドロー4"
    elif word in ["+1", "d1", "dobon1", "英孝1", "狩野英孝1"]:
        return "ドボン1"
    elif word in ["+2", "d2", "dobon2", "英孝2", "狩野英孝2"]:
        return "ドボン2"
    else:
